...
export const useGardenStore = defineStore('garden', () => {
  ...
  const filteredData = computed<GardenData[]>(() => {
    ...
    if (filters.value.districts && filters.value.districts.length > 0) {
      data = data.filter((item) => filters.value.districts!.includes(item.district));
    }

    // 开放情况筛选
    if (filters.value.openStatus && filters.value.openStatus.length > 0) {
      data = data.filter((item) => filters.value.openStatus!.includes(item.openStatus));
    }

    // 文保单位级别筛选
    if (filters.value.heritageLevels && filters.value.heritageLevels.length > 0) {
      data = data.filter((item) => filters.value.heritageLevels!.includes(item.heritageLevel));
    }

    // 权属性质筛选
    if (filters.value.ownershipTypes && filters.value.ownershipTypes.length > 0) {
      data = data.filter((item) => filters.value.ownershipTypes!.includes(item.ownershipType));
    }

    // 当前用途筛选
    if (filters.value.currentUses && filters.value.currentUses.length > 0) {
      data = data.filter((item) => filters.value.currentUses!.includes(item.currentUse));
    }

    // 世界遗产筛选
    if (filters.value.isWorldHeritage !== null && filters.value.isWorldHeritage !== undefined) {
      data = data.filter((item) => item.isWorldHeritage === filters.value.isWorldHeritage);
    }
    ...
    if (filters.value.eraCategories && filters.value.eraCategories.length > 0) {
      data = data.filter((item) => filters.value.eraCategories!.includes(item.eraCategory));
    }
    ...
    if (filters.value.areaRanges && filters.value.areaRanges.length > 0) {
      data = data.filter((item) => filters.value.areaRanges!.includes(item.areaRange));
    }
  ...
  const districtStatistics = computed<DistrictStatistics[]>(() => {
    ...
    return districtData.value.map((district) => {
      ...
      // 人均开放园林数：个/万人
      const openGardenPerCapita =
        district.population > 0 ? openGardenCount / district.population : 0;
  ...
  function searchGardenByName(keyword: string): GardenData[] {
    ...
    return rawData.value.filter((item) => item.name.toLowerCase().includes(normalized));
  }
