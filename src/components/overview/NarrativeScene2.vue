...
import { getEraCategoryColor, getHeritageLevelColor } from '@/config/theme';
...
<template>
  <div class="narrative-scene-2 p-6">
    <!-- 场景标题与核心观点 -->
    <div class="mb-6">
      <h2 class="mb-2 text-2xl font-bold text-gray-900">历史谱系与认定节奏</h2>
      <div class="mb-4 text-sm text-gray-600">
        <strong>核心观点：</strong
        >认定节奏存在明显阶段性，现代园林集中在后期批次，早期园林分布更分散，体现价值认定的时序差异。
      </div>
    ...
    <div class="mb-6 grid grid-cols-2 gap-4">
      <div
        class="bg-linear-to-r rounded-lg border border-purple-200 from-purple-50 to-purple-100 p-4"
      >
        <div class="mb-1 text-xs font-medium text-purple-600">认定节奏</div>
        <div class="text-lg font-bold text-purple-800">现代园林集中在后期批次（3/4）</div>
        <div class="mt-1 text-xs text-purple-600">早期园林分布更分散，体现价值认定的时序差异</div>
      </div>
      <div class="bg-linear-to-r rounded-lg border border-amber-200 from-amber-50 to-amber-100 p-4">
        <div class="mb-1 text-xs font-medium text-amber-600">历史规模</div>
        <div class="text-lg font-bold text-amber-800">明代与宋代及以前园林面积整体更大</div>
        <div class="mt-1 text-xs text-amber-600">体现历史园林的规模优势</div>
      </div>
    </div>
    ...
    <div class="mb-6 grid grid-cols-4 gap-4">
      ...
      <MetricCard title="早期园林" :value="metrics.earlyCount" unit="座" description="宋代及以前" />
      <MetricCard title="最早园林" :value="metrics.oldestGarden" />
      <MetricCard title="公布批次数" :value="metrics.batchCount" unit="批" />
    </div>
    ...
    <div class="mb-6 grid grid-cols-2 gap-6">
      ...
      <div class="rounded-lg border border-gray-200 bg-white p-4">
        <BarChart
          ...
          height="400px"
        />
        <div class="mt-2 text-xs text-gray-500">注：清代园林占主导，现代园林占比较高</div>
      </div>
    ...
    <!-- 早期年代园林清单表格 -->
    <div class="rounded-lg border border-gray-200 bg-white p-4">
      <h3 class="mb-4 text-lg font-semibold text-gray-900">早期园林清单（宋代及以前）</h3>
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="divide-y divide-gray-200 bg-white">
            <tr v-for="garden in earlyGardens" :key="garden.name" class="hover:bg-gray-50">
      ...
      <div v-if="earlyGardens.length === 0" class="py-8 text-center text-gray-500">
        暂无早期年代园林数据
      </div>
    </div>
  </div>
