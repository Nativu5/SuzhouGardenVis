...
import { getProtectionStatusColor } from '@/config/theme';
...
const keyInsights = computed(() => {
  // 计算中/差园林中不开放的比例
  const riskGardensData = data.value.filter(
    (item) => item.protectionStatus === '中' || item.protectionStatus === '差',
  );
  const riskClosedCount = riskGardensData.filter((item) => item.openStatus === '不开放').length;
  const riskClosedRate =
    riskGardensData.length > 0
...
<template>
  <div class="narrative-scene-5 p-6">
    <!-- 场景标题与核心观点 -->
    <div class="mb-6">
      <h2 class="mb-2 text-2xl font-bold text-gray-900">保护状况与风险提示</h2>
      <div class="mb-4 text-sm text-gray-600">
        <strong>核心观点：</strong
        >保护状况为"中/差"的园林多数不开放，风险主要集中在少数区县与特定权属结构。
      </div>
    ...
    <div class="mb-6 grid grid-cols-2 gap-4">
      <div class="bg-linear-to-r rounded-lg border border-red-200 from-red-50 to-red-100 p-4">
        <div class="mb-1 text-xs font-medium text-red-600">保护与开放关联</div>
        <div class="text-lg font-bold text-red-800">
          保护状况为"中/差"的园林中，不开放比例达 {{ keyInsights.riskClosedRate }}%
        </div>
        <div class="mt-1 text-xs text-red-600">保护状况与开放性存在明显关联</div>
      </div>
      <div class="bg-linear-to-r rounded-lg border border-amber-200 from-amber-50 to-amber-100 p-4">
        <div class="mb-1 text-xs font-medium text-amber-600">风险空间集中</div>
        <div class="text-lg font-bold text-amber-800">
          风险园林集中在 {{ keyInsights.topRiskDistricts }}
        </div>
        <div class="mt-1 text-xs text-amber-600">可作为治理重点提示</div>
      </div>
    </div>
    ...
    <div class="mb-6 grid grid-cols-5 gap-4">
      ...
      <MetricCard title="状况良好" :value="metrics.goodCount" unit="座" />
      <MetricCard title="状况中等" :value="metrics.mediumCount" unit="座" />
      <MetricCard title="状况较差" :value="metrics.poorCount" unit="座" />
      <MetricCard title="风险比例" :value="metrics.riskRate" description="中+差" />
    </div>
    ...
    <div class="mb-6 grid grid-cols-2 gap-6">
      ...
      <div class="rounded-lg border border-gray-200 bg-white p-4">
        <HeatmapMatrixChart
          ...
          height="400px"
        />
        <div class="mt-2 text-xs text-gray-500">注：色深表示数量，鼠标悬停显示保护状况内开放率</div>
      </div>
      ...
      <div class="rounded-lg border border-gray-200 bg-white p-4">
        <StackedBarChart
          ...
          height="400px"
        />
        <div class="mt-2 text-xs text-gray-500">注：展示各区县保护状况构成，颜色=保护状况</div>
      </div>
    </div>

    <!-- 风险园林清单表格 -->
    <div class="rounded-lg border border-gray-200 bg-white p-4">
      <h3 class="mb-4 text-lg font-semibold text-gray-900">风险园林清单（保护状况为"中"或"差"）</h3>
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="divide-y divide-gray-200 bg-white">
            <tr v-for="garden in riskGardens" :key="garden.name" class="hover:bg-gray-50">
      ...
      <div v-if="riskGardens.length === 0" class="py-8 text-center text-gray-500">
        暂无风险园林数据
      </div>
    </div>
  </div>
