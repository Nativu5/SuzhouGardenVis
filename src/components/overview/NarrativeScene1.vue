...
const coreDistrictScatter = computed(() => {
  const scatterData = store.districtStatistics
    ...
    .map((district) => {
      ...
      const highLevelRatio =
        district.gardenCount > 0 ? (highLevelCount / district.gardenCount) * 100 : 0;
  ...
  <div class="narrative-scene-1 p-6">
    ...
    <div class="mb-6 grid grid-cols-3 gap-4">
      <div class="bg-linear-to-r rounded-lg border border-red-200 from-red-50 to-red-100 p-4">
        <div class="mb-1 text-xs font-medium text-red-600">空间集中度</div>
        <div class="text-lg font-bold text-red-800">
          姑苏区占地约 {{ keyInsights.gususAreaRatio }}，却拥有
          {{ keyInsights.gususGardenRatio }} 园林
        </div>
      ...
      >
        ...
        <div class="mt-1 text-xs text-yellow-600">遗产核心高度单极化</div>
      </div>
      <div class="bg-linear-to-r rounded-lg border border-green-200 from-green-50 to-green-100 p-4">
        <div class="mb-1 text-xs font-medium text-green-600">高等级遗产</div>
        <div class="text-lg font-bold text-green-800">核心区高等级遗产占比显著更高</div>
        <div class="mt-1 text-xs text-green-600">全国+省级占比 {{ metrics.highLevelRatio }}</div>
      </div>
    </div>
    ...
    <div class="mb-6 grid grid-cols-4 gap-4">
      ...
      <MetricCard title="世界遗产" :value="metrics.heritageCount" unit="座" />
      <MetricCard title="高等级占比" :value="metrics.highLevelRatio" />
      <MetricCard title="最高集中度" :value="metrics.concentrationRatio" />
    </div>
    ...
    <!-- 世界遗产园林清单表格 -->
    <div class="rounded-lg border border-gray-200 bg-white p-4">
      <h3 class="mb-4 text-lg font-semibold text-gray-900">世界遗产园林清单</h3>
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="divide-y divide-gray-200 bg-white">
            <tr v-for="garden in worldHeritageGardens" :key="garden.name" class="hover:bg-gray-50">
      ...
      <div v-if="worldHeritageGardens.length === 0" class="py-8 text-center text-gray-500">
        暂无世界遗产园林数据
      </div>
    </div>
  </div>
