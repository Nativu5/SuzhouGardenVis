...
// 按建造年代构建 Treemap 数据（Top6 + 其他）
const eraAreaTreemap = computed(() => {
  const result = buildEraAreaTreemap(data.value, 6);
  return {
    data: result,
  ...
  <div class="narrative-scene-4 p-6">
    ...
    <div class="mb-6">
      ...
      <div class="mb-4 text-sm text-gray-600">
        <strong>核心观点：</strong
        >园林规模呈显著长尾分布，资源高度集中于姑苏区与明清时期，Top10大园林贡献近半总面积。
      </div>

      <!-- 阅读路径提示 -->
      <div class="rounded-lg border border-blue-200 bg-blue-50 p-3 text-xs text-blue-800">
        <strong>阅读路径：</strong>
        先看"累积面积曲线"直观感受资源集中度，再看"面积区间分布（权属）"理解结构差异，最后看"年代面积
        Treemap"把握时代规模格局
      </div>
    </div>
    ...
    <div class="mb-6 grid grid-cols-2 gap-4">
      ...
      >
        ...
        <div class="mt-1 text-xs text-orange-600">少数大园林占据近半面积</div>
      </div>
      <div class="bg-linear-to-r rounded-lg border border-teal-200 from-teal-50 to-teal-100 p-4">
        <div class="mb-1 text-xs font-medium text-teal-600">规模与开放性</div>
        <div class="text-lg font-bold text-teal-800">
          开放园林平均面积 {{ keyInsights.openAvgArea.toLocaleString() }} ㎡ > 不开放
          {{ keyInsights.closedAvgArea.toLocaleString() }} ㎡
        </div>
        <div class="mt-1 text-xs text-teal-600">规模与开放正相关</div>
      </div>
    </div>
    ...
    <div class="mb-6 grid grid-cols-4 gap-4">
      ...
      <MetricCard title="总面积" :value="metrics.totalArea" unit="㎡" />
      <MetricCard title="平均面积" :value="metrics.avgArea" unit="㎡" />
      <MetricCard title="最大面积" :value="metrics.maxArea" unit="㎡" />
    </div>
    ...
    <div class="mb-6 grid grid-cols-2 gap-6">
      ...
      <div class="col-span-2 rounded-lg border border-gray-200 bg-white p-4">
        <CumulativeAreaChart
          title="累积面积分布（按开放情况分组）"
          :series-data="cumulativeAreaData"
          height="350px"
        />
      ...
      <div class="col-span-2 rounded-lg border border-gray-200 bg-white p-4">
        <HistogramChart
          title="面积区间分布（按权属性质分层）"
          :intervals="areaOwnershipData.intervals"
          :series="areaOwnershipData.series"
          x-axis-name="面积区间 (㎡)"
          y-axis-name="园林数量"
          height="400px"
        />
      ...
      <div class="rounded-lg border border-gray-200 bg-white p-4">
        <TreemapChart
          title="建造年代面积结构（Top6 + 其他）"
          :data="eraAreaTreemap.data"
          :color-palette="eraAreaTreemap.colors"
          height="400px"
        />
        <div class="mt-2 text-xs text-gray-500">
          注：每个朝代以总面积为大矩形，内部展示面积 Top6
          园林与“其他”汇总，体现不同时代的规模结构差异
        </div>
      </div>
    </div>
    ...
    <!-- 面积 Top10 园林表格 -->
    <div class="rounded-lg border border-gray-200 bg-white p-4">
      <h3 class="mb-4 text-lg font-semibold text-gray-900">面积 Top10 园林</h3>
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="divide-y divide-gray-200 bg-white">
            <tr v-for="(garden, index) in top10Gardens" :key="garden.name" class="hover:bg-gray-50">
      ...
      <div v-if="top10Gardens.length === 0" class="py-8 text-center text-gray-500">
        暂无园林数据
      </div>
    </div>
  </div>
