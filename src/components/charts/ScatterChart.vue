...
const chartOption = computed<EChartsOption>(() => {
  ...
  const option: EChartsOption = {
    ...
    legend: props.showLegend && groupedData.value.length > 1 ? {
    ...
    grid: {
      left: '10%',
      right: '10%',
      top: props.title ? (props.showLegend && groupedData.value.length > 1 ? 100 : 60) : (props.showLegend && groupedData.value.length > 1 ? 60 : 40),
      bottom: '15%',
      containLabel: true
    },
    xAxis: {
    ...
    yAxis: props.yAxisType === 'category' ? {
      type: 'category',
