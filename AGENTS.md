...
- 场景4“面积区间分布”改为按权属性质分层，并更新对应文案与配色引用
- 新增面积区间×权属性质分层统计方法，供概览图表使用
- 场景4“建造年代平均面积”替换为 Treemap（朝代总面积，Top3+其他）
- 新增 TreemapChart 组件并引入 ECharts treemap 模块与数据构建函数
- 修复 Treemap 数据构建的类型字面量问题，确保 TS 类型检查通过
- 补充 ECharts GraphicComponent 并调整 Treemap 叶节点深度，确保内部元素显示
- 优化 Treemap 层级显示与朝代标签（upperLabel 格式、边界与间距）
- Treemap 的 TopN 调整为 6，降低“其他”面积占比

**下一步工作：**

- 如需进一步统一叙事口径，可调整场景4的“规模与开放性”提示文案或补充权属结构洞察
- 视需要优化 Treemap 标签策略（例如仅显示 Top3 园林名或加强 tooltip 信息）
